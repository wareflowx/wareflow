...
### Command Palette
Universal search accessible via `Cmd+K` / `Ctrl+K`. Allows users to quickly navigate to any project. Essential for power users familiar with keyboard-driven workflows.

### Responsive
- Desktop: Full `max-w-5xl` centered layout
- Tablet: Adjusted layout for touch interaction
- Mobile: Streamlined view for quick access